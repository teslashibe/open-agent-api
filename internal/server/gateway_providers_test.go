...
func TestModelsFiltersDisabledProviders(t *testing.T) {
	...
	for _, model := range body.Data {
		if provider := codex.ProviderForModel(openai.ResolveModelAlias(model.ID).UpstreamModel); provider == codex.ProviderClaude {
			t.Fatalf("model list contains claude id %q", model.ID)
		}
		if strings.Contains(model.ID, "claude") || model.ID == "sonnet" || model.ID == "opus" || model.ID == "haiku" || model.ID == "fable" {
			t.Fatalf("model list contains claude alias %q", model.ID)
		}
