...
func TestAgentQueueFullReturns429(t *testing.T) {
	...
	for _, want := range []string{"agent_queue_full request_id=", "key_mode=cursor:x-forwarded-for", "key_hash=", "limit=0"} {
		if !strings.Contains(logs.String(), want) {
			t.Fatalf("logs = %q, want %q", logs.String(), want)
		}
	}
...
func TestAgentQueueTimeoutReturns429(t *testing.T) {
	...
	for _, want := range []string{"agent_queue_timeout request_id=", "key_mode=cursor:x-forwarded-for", "key_hash=", "wait_ms="} {
		if !strings.Contains(logs.String(), want) {
			t.Fatalf("logs = %q, want %q", logs.String(), want)
		}
	}
