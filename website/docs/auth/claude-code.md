...
`CLAUDE_CODE_OAUTH_TOKEN` is injected from Secret `codex-chat-api-secrets` (env, not a file mount). Rotate by updating the SOPS secret and restarting the Deployment.
